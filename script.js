...
const admissionData = { /* … same as before … */ };
const programmeRequirements = { /* … same as before … */ };
const studyFields = [ /* … same as before … */ ];
const resources = [ /* … same as before … */ ];
...
// STORAGE
// ============================================================
const STORAGE_KEYS = { SAVED: 'mytertiary_saved', VIEWED: 'mytertiary_viewed', COMPARE: 'mytertiary_compare', LAST_READ: 'mytertiary_last_read' };
...
function requestNotificationPermission() {
    ...
    Notification.requestPermission().then(permission => {
        if (permission === 'granted') {
            ...
            setTimeout(() => {
                if (Notification.permission === 'granted') {
                    new Notification('MyTertiary ZA', { body: '🎓 New updates for 2027 applications!', icon: 'https://ui-avatars.com/api/?name=MT&background=f5c842&color=fff&size=64&rounded=true' });
                }
            }, 10000);
        } else {
...
function sendTestNotification() {
    if (Notification.permission === 'granted') {
        new Notification('MyTertiary ZA', { body: '✅ Notifications working!', icon: 'https://ui-avatars.com/api/?name=MT&background=f5c842&color=fff&size=64&rounded=true' });
    }
}
...
// ============================================================
const adminMessages = [
    { id: 1, sender: "Admin Sia13", date: "2026-09-04", time: "08:30", message: "Good morning! Don't forget to check the new university requirements for 2027." },
    { id: 2, sender: "Admin Sia13", date: "2026-09-04", time: "12:15", message: "Reminder: The APS calculator has been updated with the latest guidelines." },
    { id: 3, sender: "Admin Sia13", date: "2026-09-03", time: "14:00", message: "Welcome to MyTertiary! Start by exploring the 'What Can I Study?' section." },
    { id: 4, sender: "Admin Sia13", date: "2026-09-04", time: "16:45", message: "University applications for 2027 are opening soon. Get your documents ready!" }
];
...
function openInbox() {
    ...
    if (todaysMessages.length === 0) {
        ...
        todaysMessages.forEach(msg => {
            html += `<div class="inbox-item"><div class="inbox-avatar"><img src="Sia13.jpeg" alt="Admin" onerror="this.style.display='none';this.nextElementSibling.style.display='flex';"><span class="avatar-fallback" style="display:none;"><i class="fas fa-user-shield"></i></span></div><div class="inbox-content"><div class="inbox-header"><span class="inbox-sender"><span class="sender-avatar-sm"><img src="Sia13.jpeg" alt="Admin" onerror="this.style.display='none';this.nextElementSibling.style.display='flex';"><span class="fallback" style="display:none;"><i class="fas fa-user-shield"></i></span></span>${escapeHTML(msg.sender)}</span><span class="inbox-time"><i class="far fa-clock"></i> ${escapeHTML(msg.time)}</span></div><div class="inbox-message">${escapeHTML(msg.message)}</div></div></div>`;
        });
...
function renderPeriodStatus(inst) {
    ...
    let statusClass = 'unknown', statusText = 'Typical Period';
    if (status === 'open') { statusClass = 'open'; statusText = 'Application Period'; }
...
function renderCards(list) {
    ...
    document.querySelectorAll('.share-btn').forEach(btn => {
        btn.addEventListener('click', function(e) {
            ...
            if (navigator.share) {
                navigator.share({ title: inst.name, text: `Check out ${inst.name} – ${inst.description}`, url: inst.website }).catch(() => {});
            } else {
                navigator.clipboard?.writeText(inst.website).then(() => showToast('🔗 Link copied!')).catch(() => showToast('🔗 Share: ' + inst.website));
            }
        });
    });
...
function showToast(msg) {
    ...
    div.style.cssText = `position:fixed;bottom:20px;left:50%;transform:translateX(-50%);background:var(--gray-900);color:white;padding:8px 16px;border-radius:30px;font-size:0.8rem;font-weight:600;box-shadow:0 4px 20px rgba(0,0,0,0.2);z-index:3000;max-width:90%;text-align:center;animation:fadeInUp 0.3s ease;`;
    div.textContent = msg;
    document.body.appendChild(div);
    setTimeout(() => { div.style.opacity = '0'; div.style.transition = 'opacity 0.3s'; setTimeout(() => div.remove(), 400); }, 2500);
}
const toastStyle = document.createElement("style");
toastStyle.textContent = `@keyframes fadeInUp { from { opacity:0; transform:translateX(-50%) translateY(20px); } to { opacity:1; transform:translateX(-50%) translateY(0); } }`;
...
document.getElementById('exploreUniBtn').addEventListener('click', function(e) { e.preventDefault(); setTab('all'); });
...
function buildCarousel() {
    ...
    institutions.forEach((inst, idx) => {
        ...
        slide.addEventListener('click', () => { document.getElementById('directory').scrollIntoView({ behavior: 'smooth' }); searchInput.value = inst.name; filterInstitutions(); });
...
function renderFinder() {
    ...
    document.querySelectorAll('.finder-option').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (idx > -1) { selectedInterests.splice(idx, 1); btn.classList.remove('selected'); } else { selectedInterests.push(id); btn.classList.add('selected'); }
        });
    });
...
function showRecommendations() {
    ...
    if (recommendedFields.length === 0) {
        finderResults.innerHTML = `<div class="result-hint"><i class="fas fa-lightbulb"></i> Based on your interests, we recommend exploring all study fields. Try selecting different interests.</div>`;
        return;
    }
...
function createSubjectEntry(name = '', percentage = '', isFixed = false) {
    ...
    if (!isFixed) {
        removeBtn.addEventListener('click', () => {
            ...
            if (total > 1 || document.querySelectorAll('.subject-entry').length > 1) { div.remove(); updateAddButton(); } else alert('You need at least one subject (Life Orientation is fixed).');
        });
    }
    return div;
}
...
addSubjectBtn.addEventListener('click', () => {
    const total = document.querySelectorAll('.subject-entry').length;
    if (total < MAX_SUBJECTS) { subjectEntriesContainer.appendChild(createSubjectEntry('', '', false)); updateAddButton(); } else alert(`You can add up to ${MAX_SUBJECTS} subjects total (including Life Orientation).`);
});
...
function calculateAPS() {
    ...
    entries.forEach(entry => {
        ...
        if (isNaN(perc) || perc < 0 || perc > 100) { percInput.style.borderColor = '#ef4444'; hasError = true; } else {
    ...
    let guidanceHTML = '';
    if (totalAPS >= 40) guidanceHTML = `<strong>Excellent! </strong>You are likely eligible for admission to most universities and competitive programmes. Consider institutions like UCT, Wits, Stellenbosch, UP and others.`;
    else if (totalAPS >= 35) guidanceHTML = `<strong>Good! </strong>You qualify for many programmes at universities such as UJ, NWU, UKZN, UWC. Check specific faculty requirements.`;
    else if (totalAPS >= 30) guidanceHTML = `<strong>Average. </strong>You may be eligible for some programmes at universities and most universities of technology. Consider CPUT, DUT, TUT, VUT and others.`;
    else if (totalAPS >= 25) guidanceHTML = `<strong>Below average. </strong>You may not meet the minimum APS for many university programmes, but universities of technology and some foundation programmes may be options. Check with institutions directly.`;
    else guidanceHTML = `<strong>Low APS. </strong>You may not qualify for most degree programmes. Consider universities of technology, bridging courses, or improving your results. Speak to a career counsellor.`;
    apsGuidance.innerHTML = guidanceHTML;
    apsResult.style.display = 'block';
}
calculateBtn.addEventListener('click', calculateAPS);
resetBtn.addEventListener('click', () => { renderDefaultSubjects(); apsResult.style.display = 'none'; document.querySelectorAll('.subject-entry input').forEach(inp => inp.style.borderColor = ''); });
...
const dailyMessages = [
    ...
    { title: "Courage", message: "Fortune Favours The Brave!", extra: "Take the next step. Your future starts with the decisions you make today." },
    ...
    { title: "Hope", message: "Your future is still being written.", extra: "Keep your head up. Your story is not over." },
    { title: "Strength", message: "Keep Your Head Up, You're Very Close.", extra: "Difficult roads often lead to beautiful destinations." },
...
window.addEventListener('scroll', () => {
    if (window.scrollY > 300) { floatingHomeBtn.classList.add('visible'); } else { floatingHomeBtn.classList.remove('visible'); }
});
...
function renderDashboard() {
    ...
    if (savedInsts.length === 0) {
        html += `<div class="dashboard-empty">You haven't saved any universities yet. Browse the <a href="#directory" style="color:var(--gold-dark);font-weight:600;" onclick="closeDashboard();">Universities</a> section and click "Save".</div>`;
    } else {
        html += `<ul class="dashboard-list">`;
        savedInsts.forEach(inst => {
            html += `<li><span class="univ-name">${escapeHTML(inst.name)}</span><span class="univ-action" onclick="closeDashboard(); document.getElementById('directory').scrollIntoView({behavior:'smooth'}); searchInput.value='${escapeHTML(inst.name)}'; filterInstitutions();">View</span></li>`;
        });
        html += `</ul>`;
    }
...
document.getElementById('inboxModal')?.addEventListener('click', function(e) { if (e.target === this) closeInbox(); });
document.addEventListener('keydown', function(e) { if (e.key === 'Escape' && document.getElementById('inboxModal').classList.contains('show')) closeInbox(); });
...
console.log('✅ MyTertiary ZA — Fully responsive, inbox badge clears, notifications ready, email & TikTok added.');